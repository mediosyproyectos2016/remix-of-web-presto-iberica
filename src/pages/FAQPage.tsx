      ...
      <main className="pt-32 pb-20">
        <section className="container-presto mb-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display mt-4 mb-6">Preguntas Frecuentes</h1>
        ...
        <section className="container-presto mb-12">
          <div className="relative max-w-2xl">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/20 text-lg"
            />
          </div>
        </section>
            ...
            <div className="space-y-4 mb-16">
              ...
              <Accordion type="single" collapsible className="space-y-4">
                {filteredQuestions.map((faq, idx) => (
                  <AccordionItem key={idx} value={`search-${idx}`} className="border border-border rounded-xl px-6 data-[state=open]:bg-muted/30">
                ...
                <nav className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0">
                  {faqCategories.map((cat) => (
                    <button
                      ...
                      className={`px-4 py-3 rounded-lg text-left whitespace-nowrap transition-colors ${
                  ...
                  <motion.div key={currentCategory.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <Accordion type="single" collapsible className="space-y-4">
                      {currentCategory.questions.map((faq, idx) => (
                        <AccordionItem key={idx} value={`item-${idx}`} className="border border-border rounded-xl px-6 data-[state=open]:bg-muted/30">
                          <AccordionTrigger className="text-left hover:no-underline py-4">
                            <span className="font-medium">{faq.q}</span>
                          </AccordionTrigger>
                          <AccordionContent className="text-muted-foreground pb-4">{faq.a}</AccordionContent>
        ...
        <section className="container-presto mt-16">
          <div className="bg-muted/50 rounded-2xl p-8 md:p-12 text-center">
            ...
            <a href="/contacto" className="inline-flex items-center justify-center px-6 py-3 bg-primary text-primary-foreground rounded-lg font-medium hover:bg-primary/90 transition-colors">
